...
#先引入配置文件，将数据引入，建立一个数据评判标准，再载入训练模型，创建trainer训练器用于利用训练模型训练数据
def main(cfg, model_cfg):
    ...
    data = load_data(cfg)
    if cfg.uda_mode:
        unsup_criterion = nn.KLDivLoss(reduction='none')  #kl散度即相对熵  把每一个元素的损失值算出来
    ...
    sup_criterion = nn.CrossEntropyLoss(reduction='none') #交叉熵
...
#损失函数，logits是一个数学向量，是一个未归一化的概率，作为softmax层的输入
